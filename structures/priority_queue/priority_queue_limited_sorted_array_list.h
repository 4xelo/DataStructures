...
namespace structures
{
    ...
    {
        if (this != &other) {
            auto otherList = dynamic_cast<PriorityQueueLimitedSortedArrayList<T>&>(other);
            PriorityQueueSortedArrayList<T>::assign(otherList);
    ...
    void PriorityQueueLimitedSortedArrayList<T>::push(int priority, const T& data)
    {
        if (PriorityQueueList<T>::list_->size() < capacity_)
            ...
            throw std::logic_error("PriorityQueueLimitedSortedArrayList<T>::push: Priority queue has exceeded its capacity");
    }
    ...
    inline PriorityQueueItem<T>* PriorityQueueLimitedSortedArrayList<T>::pushAndRemove(int priority, T data)
    {
        ...
        if (PriorityQueueList<T>::size() > capacity_)
            item = PriorityQueueList<T>::list_->removeAt(0);
    ...
    {
        if (capacity >= PriorityQueueList<T>::size() && capacity > 0)
        {
            capacity_ = capacity;
            return true;
        }
