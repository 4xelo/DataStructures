...
namespace structures
{
	...
	void Heap<T>::push(int priority, const T& data)
	{
		...
		// otec = (index -1) /2 v kcestnom strome deleno k
	...
	{
		return (index - 1) / 2;//c++ ma celociselne delenie, niektore jazyky to takto nemaju, treba davat pozor,
	}
	...
	inline int Heap<T>::getGreaterSonIndex(int index)
	{
		...
		int indexSize = PriorityQueueList<T>::list_->size() - 1;
		if (indexLeft >= indexSize)
			return -1;
		else {
			if (indexRight >= indexSize)
				return indexLeft;
			else {
