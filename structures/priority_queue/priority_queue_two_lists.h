...
namespace structures
{
	...
	Structure& PriorityQueueTwoLists<T>::assign(Structure& other)
	{
		if (this != &other)
		{
			auto& otherTwoList = dynamic_cast<PriorityQueueTwoLists<T>&>(other);
			this->clear();
			shortList_->assign(*otherTwoList.shortList_);

			for (PriorityQueueItem<T>* item : *otherTwoList.longList_) {
	...
	T PriorityQueueTwoLists<T>::pop()
	{
		...
		if (shortList_->isEmpty() && !longList_->isEmpty()) {
			...
			if (defaultSize <= longList_->size()) {
				defaultSize = (int)sqrt(longList_->size());
			}

			shortList_->trySetCapacity(defaultSize);

			auto pomLongList = new LinkedList<PriorityQueueItem<T>*>();
			...
			while (!longList_->isEmpty()) {
				...
				{
					pomLongList->add(removedItem);
				}
			}
			delete longList_;
			longList_ = pomLongList;
