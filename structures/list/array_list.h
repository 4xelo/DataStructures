...
namespace structures
{
	...
	inline bool ArrayList<T>::equals(Structure& other)
	{
		...
		if (size_ != other.size())
			return false;
		ArrayList<T>& newArrayList = dynamic_cast<ArrayList<T>&>(other);

		for (int i = 0; i < size_; i++)
		{
			if (at(i) == newArrayList.at(i))
				continue;
			return false;
		}
		return true;
	...
	{
		return new ArrayListIterator(this, static_cast<int>(size_));//size kvoli podmienke vo whilu ak by sme dali size -1 tak by sme
		//vyskocili
	}
