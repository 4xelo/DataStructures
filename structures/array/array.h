...
namespace structures
{
	...
	Array<T>::~Array()
	{
		delete vector_;//cez delete lebo sme vytvarali novy objekt, ak by sme to alokovali cez malloc/calloc tak pouzivame free
		vector_ = nullptr;
		size_ = 0;
	}
	...
	{
		if (this != &other) //kontrola identity, ak niesom identicky tak:
		{
			auto& otherArray = dynamic_cast<Array<T>&>(other);	//pretypujem other na Array<T>&
			if (size_ == otherArray.size_) //kontrolujem ci maju polia rovnaku velkost
				vector_->assign(*(otherArray.vector_)); //ano -> necham priradit vektory
				//vector_operator=(*otherArray.vector_)
				//*vector_ = *otherArray.vector_
			else
				throw std::invalid_argument("Array sizes differ"); //nie -> vynimka!

		}
		return *this;//vrati presne taky objekt aky chceme
	}
	...
	{
		size_t indexVoVektore = mapFunction(index);
		byte* bytePtr = vector_->getBytePointer(indexVoVektore);
		T* ptrT = reinterpret_cast<T*>(bytePtr);
		return *ptrT;
	}
